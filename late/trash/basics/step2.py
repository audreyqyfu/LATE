...
# read data #
file = "../../data/splat_v1-1-2/splat.OneGroup.norm.log.B.hd5" #data need imputation
...
# regression #
gene_index=2 #can loop in range(n)
...
def focusFnn(x):
    ...
    with tf.name_scope("focusFnn"):
        layer_1 = tf.nn.relu(tf.add(tf.matmul(x, weights['decoder_w1']),
                                       biases['decoder_b1']))
        layer_2 = tf.nn.relu(tf.add(tf.matmul(layer_1, tf.reshape(weights['decoder_w2'][:,gene_index], [weights['decoder_w2'].get_shape().as_list()[0], 1]) ),
                                       biases['decoder_b2'][gene_index]))
...
optimizer = tf.train.RMSPropOptimizer(learning_rate).\
    minimize(cost_fnn, var_list=decoder_train_varList)  # frozen other variables
print("# Updated layers: ", "focusFnn\n")
...
print("medium benchmark_pearsonr for gene ", gene_index, " in training cells :", medium_corr_one_gene(df2_train.values[:,gene_index:(gene_index+1)], h_train))
print("medium benchmark_pearsonr for gene ", gene_index, " in valid cells:", medium_corr_one_gene(df2_valid.values[:,gene_index:(gene_index+1)], h_valid))
...
for epoch in range(1, training_epochs+1):
    ...
    if (epoch == 1) or (epoch % display_step == 0):
        ...
        corr_train = medium_corr_one_gene(df2_train.values[:, gene_index:(gene_index + 1)], h_train)
        corr_valid = medium_corr_one_gene(df2_valid.values[:, gene_index:(gene_index + 1)], h_valid)
        print("medium benchmark_pearsonr for gene ", gene_index, " in training cells :", corr_train)
        print("medium benchmark_pearsonr for gene ", gene_index, " in valid cells:", corr_valid)
    ...
    if (epoch == 1) or (epoch % snapshot_step == 0) or (epoch == training_epochs):
        ...
        df_h_input = pd.DataFrame(data=h_input, columns=[gene_index], index=df.index)
        save_hd5(df_h_input, log_dir+"/h."+str(gene_index)+".hd5")

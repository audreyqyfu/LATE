...
batch_size = 256
sd = 0.001 #stddev for random init
...
def focusFnn(x):
    ...
    with tf.name_scope("focusFnn"):
        ...
        layer_2 = tf.nn.relu(tf.add(tf.matmul(layer_1, weights2['fnn_w2']),
                                       biases2['fnn_b2']))
        variable_summaries('decoder_w1', weights['decoder_w1'])
        ...
        variable_summaries('fnn_a2', layer_2)
    return [layer_2, layer_1]
...
biases2 = {
    'fnn_b1': tf.Variable(tf.random_normal([n_hidden_1], stddev=sd), name='fnn_b1'),
    'fnn_b2': tf.Variable(tf.ones([1])/100, name='fnn_b2')
}
...
[focusFnn_op, focusFnn_layer1] = focusFnn(encoder_op)  # for one gene a time prediction
[decoder_op, decoder_layer1] = decoder(encoder_op)  # for pearson correlation of the whole matrix #bug (8092, 0)
...
optimizer = (
    tf.train.GradientDescentOptimizer(learning_rate).
    minimize(cost_fnn, var_list=[weights2['fnn_w2'], biases2['fnn_b2']])
)# frozen other variables
print("# Updated layers: ", "last layer of fnn\n")
...
for epoch in range(1, training_epochs+1):
    ...
    if (epoch == 1) or (epoch % display_step == 0):
        ...
        print("cost_fnn_batch=", "{:.9f}".format(cost_batch),
              "cost_fnn_train=", "{:.9f}".format(cost_train),
              "cost_fnn_valid=", "{:.9f}".format(cost_valid)
              )
        ...
        # # Test weights update for weights, and biases
        # [cost_decoder_train, h_decoder_train] = sess.run([cost_decoder, decoder_op], feed_dict={X: df_train.values})
        # corr_decoder_train = medium_corr(h_decoder_train, df_train.values)
        # print("cost_decoder_train: shouldn't change", cost_decoder_train)
        # print("corr decoder train: shouldn't change", corr_decoder_train)
        #
        # fnn_layer1_activations_arr = sess.run(focusFnn_layer1, feed_dict={X: df_train.values})
        # decoder_layer1_activations_arr = sess.run(decoder_layer1, feed_dict={X: df_train.values})
        # if np.array_equal(fnn_layer1_activations_arr, decoder_layer1_activations_arr):
        #     print('equal, weights was transferred, because the activations in L3 layer are same')

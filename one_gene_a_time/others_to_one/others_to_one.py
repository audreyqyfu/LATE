...
# Parameters #
j = 1  #gene index for the target gene
learning_rate = 0.001
...
biases = {
    'encoder_b1': tf.Variable(tf.ones([n_hidden_1])),
    'encoder_b2': tf.Variable(tf.ones([n_hidden_2])),
    'decoder_b1': tf.Variable(tf.ones([n_hidden_1])),
    'decoder_b2': tf.Variable(tf.ones([1]))
}
...
optimizer = (
    ...
    minimize(cost, var_list=fine_tune_varList)
...
for epoch in range(1, training_epochs+1):
    ...
    #           medium_corr_one_gene(df2_valid.values[:, j:j + 1], h_valid))
    #     # save predictions
    #     h_input = sess.run(y_pred, feed_dict={X: df.values})
    #     print("medium pearsonr in all imputation cells: ", medium_corr_one_gene(df2.values[:,j:j+1], h_input))

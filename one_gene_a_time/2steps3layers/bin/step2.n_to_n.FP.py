...
learning_rate = 0.0  # todo: change back to 0.0001
training_epochs = 0  # todo: this version re-train skipped
batch_size = 256
...
decoder_w1 = sess.run(decoder_params['w1'])  #500, 1
decoder_b1 = sess.run(decoder_params['b1'])  #1
...
def visualization_of_dfs():
    max, min = scimpute.max_min_element_in_arrs([df_valid.values, df2_valid.values, h_valid, df.values, df2.values, h])
    scimpute.heatmap_vis(df_valid.values, title='df.valid.all'+Aname, xlab='genes', ylab='cells', vmax=max, vmin=min)
    scimpute.heatmap_vis(df2_valid.values, title='df2.valid.all'+Bname, xlab='genes', ylab='cells', vmax=max, vmin=min)
...
hist = scimpute.gene_corr_hist(h_valid, df2_valid.values,
                                  fprefix='hist gene-corr, valid, step2(FP, skipped re-train)',
...
for j in [0, 1, 200, 201, 400, 401, 600, 601, 800, 801, 998, 999]:
    scimpute.scatterplot2(df2_valid.values[:, j], h_valid[:, j],
                          title=str('scatterplot, gene-' + str(j) + ', valid, step2(FP, skipped re-train)'),
                          xlabel='Ground Truth ' + Bname,
                          ylabel='Prediction ' + Aname,
                          range=[0, 6]  # todo: change hardcode
                          )
...
corrcoef_matrix_gene_wise0 = np.corrcoef(df2, rowvar=False)
scimpute.hist_arr_flat(corrcoef_matrix_gene_wise0, title=Bname + "corr_gene_wise")
scimpute.heatmap_vis(corrcoef_matrix_gene_wise0, title=Bname + "corr_gene_wise.heatmap.png", vmin=-1, vmax=1)
for i,j in list:
    scimpute.scatterplot2(df2.ix[:,i], df2.ix[:,j], title="Gene"+str(i+1)+'vs Gene'+str(j+1)+'in ' + Bname,
                          xlabel='Gene'+str(i+1), ylabel='Gene'+str(j+1))
    
# For input data #
corrcoef_matrix_gene_wise1 = np.corrcoef(df, rowvar=False)
scimpute.hist_arr_flat(corrcoef_matrix_gene_wise1, title=Aname + "corr_gene_wise")
scimpute.heatmap_vis(corrcoef_matrix_gene_wise1, title=Aname + "corr_gene_wise.heatmap.png", vmin=-1, vmax=1)
for i,j in list:
    scimpute.scatterplot2(df.ix[:,i], df.ix[:,j], title="Gene"+str(i+1)+'vs Gene'+str(j+1)+'in ' + Aname,
                          xlabel='Gene'+str(i+1), ylabel='Gene'+str(j+1))

# for prediction #
corrcoef_matrix_gene_wise = np.corrcoef(h, rowvar=False)
scimpute.hist_arr_flat(corrcoef_matrix_gene_wise, title="step2(FP, skipped retrain).imputation.corr_gene_wise")
scimpute.heatmap_vis(corrcoef_matrix_gene_wise, title="step2(FP, skipped retrain).imputation.corr_gene_wise.heatmap.png", vmin=-1, vmax=1)
for i,j in list:
    scimpute.scatterplot2(h[:,i], h[:,j], title="Gene"+str(i+1)+'vs Gene'+str(j+1)+' (step2 [FP, skipped retrain], prediction)',
                          xlabel='Gene'+str(i+1), ylabel='Gene'+str(j+1))

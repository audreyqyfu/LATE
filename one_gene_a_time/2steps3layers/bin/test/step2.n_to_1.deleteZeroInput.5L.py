...
for j in j_lst:
    ...
    def focusFnn(x):
        with tf.name_scope("Decoder"):
            ...
            layer_1 = tf.nn.relu(tf.add(tf.matmul(x_drop, focusFnn_params['w1']),
                                        focusFnn_params['b1']))
            layer_1_drop = tf.nn.dropout(layer_1, keep_prob_hidden)
            layer_2 = tf.nn.relu(tf.add(tf.matmul(layer_1_drop, focusFnn_params['w2']),
                                        focusFnn_params['b2']))
            ...
            variable_summaries('fnn_w2', focusFnn_params['w2'])
            variable_summaries('fnn_b2', focusFnn_params['b2'])
    ...
    focusFnn_params = {
        'w1': tf.Variable(tf.random_normal([n_hidden_2, n_hidden_1], stddev=sd), name='focusFnn_w1'),
        'b1': tf.Variable(tf.random_normal([n_hidden_1], mean=30 * sd, stddev=sd), name='focusFnn_b1'),
        'w2': tf.Variable(tf.random_normal([n_hidden_1, 1], stddev=sd), name='focusFnn_w2'),
        'b2': tf.Variable(tf.random_normal([1], mean=30 * sd, stddev=sd), name='focusFnn_b2')
    }

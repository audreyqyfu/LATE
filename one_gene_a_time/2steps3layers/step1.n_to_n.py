...
def scatterplot2(x, y, title, xlabel, ylabel):
    corr = pearsonr(h_valid[:, j:j + 1], df_valid.values[:, j:j + 1])
    plt.plot(x, y, 'o')
    plt.title(str(title+"\ncorr"))
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
...
j = 3
h_valid = sess.run(y_pred, feed_dict={X: df_valid.values})
scatterplot2(df2_valid.values[:, j:j + 1], h_valid[:, j:j + 1],
             'Ground Truth vs Prediction for gene-j in Validation set, step1',
             'Ground Truth B', 'Prediction from B.msk')

hist = pearsonr_hist(h_valid, df2_valid.values, title="corr cells valid, step1")
print("median Pearsonr of genes: ", np.median(hist))
